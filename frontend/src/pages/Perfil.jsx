...
const Perfil = () => {
  ...
  const [avatarIdSeleccionado, setAvatarIdSeleccionado] = useState(null);
    ...
    return (
      <div className="w-[70%] mb-6">
        ...
        <div className="flex flex-col items-center h-fit w-full cursor-pointer">
          <motion.div
            ...
            onClick={() =>
              setSelectedAvatar(prev => {
                const next = !prev;
                // Si querés, al cerrar limpiamos el seleccionado
          ...
          {jugadorAvatares.length === 0 ? (
            <p>No hay Avatares en la lista.</p>
          ) : selectedAvatar ? (
            <></>
          ) : (
            <ul className="flex flex-row gap-3 mt-4 bg-white/10 w-[250px] rounded-xl">
              {jugadorAvatares.map((e) => (
                <motion.li
                  key={e.id}
                  className="border rounded-xl p-4 bg-white/10 hover:bg-white/20 mb-2 cursor-pointer"
                  whileTap={{ scale: 1.02 }}
                >
                  <span>ID: {e.id}</span>
                </motion.li>
