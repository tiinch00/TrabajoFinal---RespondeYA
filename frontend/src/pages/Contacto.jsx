      ...
      <section className='bg-gradient-to-r from-purple-700 to-indigo-800 text-white py-12 px-6 text-center rounded-2xl'>
        ...
        <p className='max-w-3xl mx-auto text-lg leading-relaxed'>
          <Trans i18nKey='descriptionWho'>
            Dєv²Plαy es una empresa creada por <b>Francisco Pandolfi</b> y <b>Martin Paredes</b>,
            dos desarrolladores web fundadores del popular juego en tiempo real{' '}
            <span className='italic'>RespondeYa!</span>
          </Trans>
        </p>
      ...
      <div className='w-full  flex items-center justify-center text-black m-5 p-5'>
        <div className='w-full max-w-md rounded-2xl shadow-lg flex flex-col p-5 bg-gradient-to-r from-purple-700 to-indigo-800 text-white py-4 px-6'>
          <form onSubmit={handleOnSubmit}>
            ...
            <button
              type='submit'
              className='w-full py-2 rounded-lg bg-green-500 hover:bg-green-600 text-white font-semibold transition duration-300 mt-4'
            >
              {t('sendEmail')}
            </button>
            <div>
              ...
              {status && (
                <p
                  className={`${status.startsWith('✅') ? 'text-green-600' : 'text-red-600'} mt-1`}
                >
                  {status}
                </p>
              )}
