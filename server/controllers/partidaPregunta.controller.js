...
const store = async (req, res) => {
  const { partida_id, pregunta_id, orden, question_text_copy, correct_option_id_copy, correct_option_text_copy } = req.body;
  ...
  // Validar orden
  if (orden < 1 || orden > 255) { // Asumiendo un rango razonable para TINYINT
  ...
  try {
    const partidaPregunta = await PartidaPregunta.create({ partida_id, pregunta_id, orden, question_text_copy, correct_option_id_copy, correct_option_text_copy });
...
const update = async (req, res) => {
  ...
  try {
    ...
    if (orden !== undefined) {
      ...
      const existingOrder = await PartidaPregunta.findOne({ where: { partida_id: partidaPregunta.partida_id, orden } });
      if (existingOrder && existingOrder.id !== id) {
        return res.status(409).json({ error: "This orden is already used for the partida" });
      }
    }
    await partidaPregunta.update({ orden, question_text_copy, correct_option_id_copy, correct_option_text_copy });
    res.json(partidaPregunta);
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error: "Internal server error" });
