import {PartidaJugador,Partida,User} from '../models/associations.js';
...
const store = async (req, res) => {
  ...
  if (!partida_id || !usuario_id || !rol) {
    return res.status(400).json({ error: "partida_id, usuario_id, and rol are required" });
  }
  ...
  try {
    const partidaJugador = await PartidaJugador.create({ partida_id, usuario_id, rol, joined_at });
